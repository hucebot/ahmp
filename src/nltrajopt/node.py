...
class Node:
    def __init__(
        ...
        # Type based on which constraints are used
    ...
    def init_node_ids(self, v_id, c_id, k):
        ...
        elif self.node_type == "vault":
            self._init_vault_constraints()
        elif self.node_type == "chimney":
            self._init_chimney_constraints()
        elif self.node_type == "stairs":
            self._init_stair_constraints()
        elif self.node_type == "handrails":
            self._init_stair_constraints() # Same as stairs
        else:
            print("Wrong node type selected")
            exit()
    ...
    def _init_state_variables(self, v_id: int) -> None:
        ...
        self.x_dim = 1 + self.nv * 3  # dt + q + vq + aq
    ...
    def _init_vault_constraints(self) -> None:
        """If using vault constraint"""
        height_dim = 3  # Foot height
        vel_dim = 3  # Foot velocity
        self.c_z_ids: Dict[str, slice] = {}
        self.c_vel_ids: Dict[str, slice] = {}

        if self.contact_phase_fnames:
            prev_slice = self.c_contact_kinematics_ids[self.contact_phase_fnames[-1]]
        else:
            prev_slice = self.c_dh_id
    ...
    def _init_chimney_constraints(self) -> None:
        """If using chimney constraint"""
        pos_dim = 1  # left/right hand/foot position
        vel_dim = 3  # Foot velocity
        self.c_z_ids: Dict[str, slice] = {}
        self.c_vel_ids: Dict[str, slice] = {}
        ...
        for fname in self.contact_fnames:
            self.c_z_ids[fname] = slice(prev_slice.stop, prev_slice.stop + pos_dim)
            prev_slice = copy(self.c_z_ids[fname])
            self.c_dim += pos_dim
    ...
    def _init_stair_constraints(self) -> None:
        """If using vault constraint"""
        height_dim = 2  # Foot height
        vel_dim = 3  # Foot velocity
        self.c_z_ids: Dict[str, slice] = {}
        self.c_vel_ids: Dict[str, slice] = {}
